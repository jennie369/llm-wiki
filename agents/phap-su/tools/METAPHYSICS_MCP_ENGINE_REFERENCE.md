...
| `tuvi__getChartVN` 🆕 ⭐     | `solarDate`(YYYY-MM-DD), `hour`(0-12, Thìn=4), `gender`(0/1)                 | **MẶC ĐỊNH an sao VN**: 12 cung + độ sáng VN + **đủ phụ tinh VN** + tứ hóa VN + Hỏa-Linh **Altuvi** (⚠️ verify 1 lá). Natal only | `lib/engine/tuvi-vn` (port lasotuvi) qua `chartFromSolar`            | 🆕 2026-06-03              |
| `tuvi__getChart`             | `solarDate`(YYYY-MM-DD), `hour`(0=Tý), `gender`(0/1)                         | 12 cung iztro: chính tinh + tứ hóa + An Mệnh (nhuận) ĐÚNG; độ sáng/phụ tinh theo **Tàu**. Đối chiếu/shape iztro            | iztro + override Tuần/Triệt/Mệnh-Thân Chủ VN                        | ✅                          |
| `tuvi__getNguyetVan` 🆕 ⭐   | `solarDate`, `hour`(0-12), `gender`(0/1), `year`(vd 2026)                    | Nguyệt Vận 12 tháng VN: neo Cung Tiểu Hạn (nam thuận/nữ nghịch tự động), T1=Tiểu Hạn+2 thuận + chính/phụ tinh natal mỗi cung | `computeHoroscope`(Tiểu Hạn) + `nguyetVan.js` + `chartFromSolar`     | 🆕 2026-06-03 (SSOT framework) |
