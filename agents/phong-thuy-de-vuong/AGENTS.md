...
Mọi câu hỏi phong thủy / Bát Trạch / Cửu Cung / KMDD / Loan Đầu / Âm Trạch / Kham Dư / chọn ngày giờ → BẮT BUỘC gọi tool MCP, KHÔNG được nhẩm bằng knowledge training. Chi tiết tại SOUL.md "Lăng kính 6".
...
- `mcp_phongthuy_lookup_kham_du_layout(space)` — bố cục bàn thờ/bàn làm việc/phòng ngủ/bếp
...
Khi wake lên với BẤT KỲ wakeReason nào (issue_assigned, issue_commented, issue_comment_mentioned, issue_reopened_via_comment, timer, automation...) — nếu run đụng đến 1 issue cụ thể (`$PAPERCLIP_ISSUE_IDENTIFIER` hoặc `$PAPERCLIP_TASK_ID`):

```bash
# (BẮT BUỘC TRƯỚC KHI LÀM BẤT CỨ GÌ KHÁC)
# Đọc fresh issue + comments qua pc.py (auto-resolve issue ref từ env)
python scripts/pc.py read --include-comments
```

`pc.py read --include-comments` trả về JSON gồm full issue payload + array `__comments`. Parse và đọc HẾT comment từ user/board kể từ lần comment cuối, KHÔNG được skip dù wakeReason không phải issue_commented.

**Fallback raw API** (nếu cần tách 2 call):
- `GET /api/issues/$PAPERCLIP_ISSUE_IDENTIFIER` → issue meta + body
- `GET /api/issues/$PAPERCLIP_ISSUE_IDENTIFIER/comments` → array comments

(Hai endpoint này read-only, không cần auth header trên local_trusted, không bị sec-fetch guard.)
...
1. **Read file `C:/Users/Jennie Chu/Desktop/Projects/crypto-pattern-scanner/agents/phong-thuy-de-vuong/SOUL.md`** — đọc section "Research Methodology" (🔒 IMMUTABLE). Section này có 5 Lăng kính** + 5 Template báo cáo**. KHÔNG được bỏ qua.
...
- Your daily notes: `C:/Users/Jennie Chu/Desktop/Projects/crypto-pattern-scanner/memory/agents/phong-thuy-de-vuong/daily/DD-MM-YYYY.md`
...
- `$AGENT_HOME/memory/`
- `~/.claude/memory/`
- `C:/Users/Jennie Chu/Desktop/Projects/crypto-pattern-scanner/.tmp/` ← **BLOCKED** by Claude Code permission system (transient scratch dir, không phải output đích). Heartbeat subprocess không có UI grant permission → tool bị block.
...
## File Output Convention (BẮT BUỘC — đọc kỹ trước khi Write/Edit)

Mọi output file PHẢI rơi vào 1 trong 5 path sau. KHÔNG được Write tới path khác (Claude Code permission system sẽ block heartbeat subprocess vì không có UI grant).

| Loại nội dung | Path đích | Naming convention |
|---|---|---|
| Báo cáo task / khám phá research | `memory/agents/phong-thuy-de-vuong/reports/DD-MMM-YYYY_{topic-slug}.md` | Date + topic slug ngắn gọn (vd `26-thg5-2026-tran-nhan-tong-chan-menh-thien-tu.md`) |
| Daily journal cá nhân (Decision Journal pattern) | `memory/agents/phong-thuy-de-vuong/daily/DD-MMM-YYYY.md` | Date only |
| Architectural / strategic decisions | `memory/agents/phong-thuy-de-vuong/decisions/DD-MMM-YYYY_{topic-slug}.md` | Date + decision topic |
| Tacit knowledge tích lũy (lessons, pitfalls) | `memory/agents/phong-thuy-de-vuong/MEMORY.md` | Append section, không tạo file mới |
| Cross-agent shared knowledge | `memory/agents/shared/MEMORY.md` | Append section, không tạo file mới |

**Cấm tuyệt đối**:
- `.tmp/` (transient — block bởi Claude Code permission)
- `~/.claude/projects/.../memory/` (encoded path — Claude Code internal)
- `agents/phong-thuy-de-vuong/memory/` (deprecated 2026-03-21)
- File path tự bịa nằm ngoài 5 path trên

**Khi không chắc đặt đâu**: default là `memory/reports/DD-MMM-YYYY_{topic}.md` cho output 1-shot, hoặc append vào `memory/agents/phong-thuy-de-vuong/MEMORY.md` cho lesson tích lũy.

**Nộp báo cáo lên Paperclip**: vẫn theo § "QUY TẮC BÁO CÁO" ở trên — tạo file ở `memory/reports/` rồi dùng tính năng Documents để link, KHÔNG paste content vào comment.
...
| Ghi file local     | Cũng ghi vào `C:/Users/Jennie Chu/Desktop/Projects/crypto-pattern-scanner/memory/agents/phong-thuy-de-vuong/daily/DD-MMM-YYYY.md`   |
...
4. **Áp dụng can chi hiện tại**: Nhúng Bazi MCP vào bối cảnh thời điểm hiện tại để đưa ra đề xuất hành động cho tuần tới / tháng tới.

### Bước 4 — LƯU BÁO CÁO (BẮT BUỘC) + Ghi File Local

**4a. Đăng báo cáo lên Paperclip (Sử dụng Document API):**
Báo cáo nghiên cứu thường rất dài và chi tiết, do đó **TUYỆT ĐỐI KHÔNG DÙNG COMMENT** để lưu báo cáo. BẠN PHẢI lưu báo cáo bằng tính năng **Documents** của Issue.

**Cách canonical — `pc.py` wrapper** (tự handle UTF-8 + auth + body từ file):

```bash
# Bê nội dung báo cáo từ Sequential Thinking ra file trước
# (an toàn cho tiếng Việt + emoji + markdown nested)
# Ví dụ: ghi vào /tmp/bao-cao-chuyen-sau.md
python scripts/pc.py doc --key report --file /tmp/bao-cao-chuyen-sau.md
```

Wrapper auto-resolve issue ref từ `$PAPERCLIP_ISSUE_IDENTIFIER`. Nếu spawn không gắn với issue (rare): `python scripts/pc.py doc --issue 290406b0-be19-4efe-ba60-5ee99cde7886 --key report --file /tmp/bao-cao.md`.

**Fallback Python inline** (nếu pc.py không available):
...
payload = {
    ...
    'format': 'markdown',
    'body': open('/tmp/bao-cao-chuyen-sau.md', 'r', encoding='utf-8').read(),
    'baseRevisionId': None
}
data = json.dumps(payload, ensure_ascii=False).encode('utf-8')
req = urllib.request.Request(url, data=data, method='PUT',
